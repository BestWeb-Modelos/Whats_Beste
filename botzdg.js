...
io.on('connection', function(socket) {
  socket.emit('message', '© BOT BESTE-WEB - Iniciado');
...
client.on('qr', (qr) => {
    ...
    qrcode.toDataURL(qr, (err, url) => {
      socket.emit('qr', url);
      socket.emit('message', '© BOT BESTE-WEB QR Code recebido, aponte a câmera  seu celular!');
    });
});

client.on('ready', () => {
    socket.emit('ready', '© BOT BESTE-WEB Dispositivo pronto!');
    socket.emit('message', '© BOT BESTE-WEB Dispositivo pronto!');
    socket.emit('qr', './check.svg')	
    console.log('© BOT BESTE-WEB Dispositivo pronto');
});

client.on('authenticated', () => {
    socket.emit('authenticated', '© BOT BESTE-WEB Autenticado!');
    socket.emit('message', '© BOT BESTE-WEB Autenticado!');
    console.log('© BOT BESTE-WEB Autenticado');
});
...
app.post('/zdg-message', [
  ...
  if (numberDDI !== "55") {
    ...
    res.status(200).json({
      status: true,
      message: 'BOT-Beste Mensagem enviada',
      response: response
    });
    }).catch(err => {
    res.status(500).json({
      status: false,
      message: 'BOT-Beste Mensagem não enviada',
      response: err.text
    });
    });
  }
...
app.post('/zdg-media', [
  ...
  if (numberDDI !== "55") {
    ...
    res.status(200).json({
      status: true,
      message: 'BOT-Beste Imagem enviada',
      response: response
    });
    }).catch(err => {
    res.status(500).json({
      status: false,
      message: 'BOT-Beste Imagem não enviada',
      response: err.text
    });
    });
  }
...
client.on('message', async msg => {
  ...
  if (msg.body !== null && msg.body === "1") {
    ...
    msg.reply("*" + nomeContato + "*, " + "essas são as principais APIs que a Beste Web vai te ensinar a usar com o WhatsApp:\nBaileys, Venom-BOT, WPPConnect, WPPWeb-JS e Cloud API (Api Oficial)\n\n*Essas são as principais integrações que a ZDG vai te ensinar a fazer com o WhatsApp:*\nBubble, WordPress (WooCommerce e Elementor), Botpress, N8N, DialogFlow, ChatWoot e plataformas como Hotmart, Edduz, Monetizze, Rd Station, Mautic, Google Sheets, Active Campaing, entre outras.");
        ...
        setTimeout(function() {
            msg.reply(`@${contact.number}` + ' seu contato já foi encaminhado para o Elton');  
            client.sendMessage('5541998306017@c.us','Contato Beste. https://wa.me/' + `${contact.number}`);
    ...
    msg.reply("Seu contato já foi encaminhado para o Elton");
  }
    ...
    delay(8000).then(async function() {
      ...
      msg.reply(saudacao + " Esse é um atendimento automático, e não é monitorado por um humano. Caso queira falar com um atendente, escolha a opção 4. \r\n\r\nEscolha uma das opções abaixo para iniciarmos a nossa conversa: \r\n\r\n*[ 1 ]* - Quero garantir minha Assinatura. \r\n*[ 4 ]- Gostaria de falar com o Elton, mas obrigado por tentar me ajudar.* \r\n");
		});
    // " Esse é um atendimento automático, e não é monitorado por um humano. Caso queira falar com um atendente, escolha a opção 4. \r\n\r\nEscolha uma das opções abaixo para iniciarmos a nossa conversa: \r\n\r\n*[ 1 ]* - Quero garantir minha Assinatura. \r\n *[ 2 ]* - O que vou receber entrando realizando a assinatura? \r\n*[ 3 ]*- Como funciona a ferramenta de BOT da Beste? \r\n*[ 4 ]- Gostaria de falar com o Elton, mas obrigado por tentar me ajudar.* \r\n*[ 5 ]*- Quero aprender a usar a ferramenta Beste.\r\n*[ 6 ]*- Quero conhecer todas as ferramentas da Beste Web\r\n*[ 7 ]*- Gostaria de conhecer Mais sobre o produto  \r\n*[ 8 ]*- In *ENGLISH* please! \r\n*[ 16 ]*- En *ESPAÑOL* por favor."
	}
});

console.log("\nA Beste Web é a oportunidade perfeita para você aprender a criar soluções incríveis usando as APIs, sem precisar de experiência prévia com programação. Com conteúdo exclusivo e atualizado, você terá tudo o que precisa para criar robôs, sistemas de atendimento e automações do zero. O curso é projetado para iniciantes e avançados, e oferece um aprendizado prático e passo a passo para que você possa criar soluções incríveis.")
